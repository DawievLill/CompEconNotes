...
md" Start our discussion on stochastic dynamic programming. Introduce the idea of uncertainty into our cake eating problem. At the end we look at value and policy function iteration for the stochastic growth model. "
...
md" These notes are almost directly from [QuantEcon](https://julia.quantecon.org/dynamic_programming/optgrowth.html). Some slight modification have been made, but the general ideas are better explained there. The reason for illustrating this example here is to utilise the interactive notebook features that Pluto offers. "

# ╔═╡ cbe8c857-ec72-48ff-93cf-61f28dda4e94
md" The algorithm for fitted value function iteration is depicted as follows:

1. Begin with array ${w_1, \ldots ,w_I}$ representing values of initial function $w$ on grid points ${y_1,\ldots,y_I}$
2. Build function $\hat{w}$ on state space $\mathbb{R}_{+}$ by interpolation, based on these data points.
3. Obtain and record the value $T\hat{w}(y_{i})$ on each grid point $y_i$
4. Unless stopping condition is satisfied, set ${w_1, \ldots , w_I} = {T\hat{w}(y_{1}), \ldots , T\hat{w}(y_{I})}$ and go to step $2$ "

# ╔═╡ 3e3a7c19-515f-4019-b85a-45c86b49ba67
md" Below is the Bellman operator "
...
function T(w, grid, β, u, f, shocks; compute_policy = false)
	...
	results = maximize.(objectives, 1e-10, grid)
